...
extension SVGXParsing: CustomDebugStringConvertible, CustomStringConvertible {
	...
	public var debugDescription: String {
		...
		SVGXParsing:
			...
			lightIntensity (0-1): \(lightIntensity?.stringValue ?? "nil")
			resolutionX: \(resolutionX?.stringValue ?? "nil")
			resolutionY: \(resolutionY?.stringValue ?? "nil")
			displayWidth (build area width dimension [screen width]): \(displayWidth?.stringValue ?? "nil")
			displayHeight (build area depth dimension [screen height]): \(displayHeight?.stringValue ?? "nil")
			machineZ (build area vertical dimension [z rod height]): \(machineZ?.stringValue ?? "nil")
			attachLayer (buildplate layer count): \(attachLayer?.stringValue ?? "nil")
			buildInLayer (transition layers from buildplate to basic layers): \(buildInLayer?.stringValue ?? "nil")
			attachTime (buildplate layer time): \(attachTime?.stringValue ?? "nil")
			baseTime (basic/normal layer time): \(baseTime?.stringValue ?? "nil")
